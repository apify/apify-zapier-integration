...
- `package.json` has an `overrides` block (`axios`, `brace-expansion`, `diff`, `flatted`, `form-data`, `js-yaml`, `lodash`, `picomatch`, `serialize-javascript`) that exists to clear `npm audit` findings from transitive deps — keep it in sync when bumping dependencies rather than removing entries.
